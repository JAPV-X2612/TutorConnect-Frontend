...
/**
 * Shared profile screen used by both learner and tutor tab groups.
 *
 * Displays: avatar, full name, email, role badge, academic info, and a
 * logout action with confirmation modal.
 *
 * Role-specific sections are hidden conditionally — additional sections for
 * a given role should be composed by extending this component rather than
 * branching on `profile.role` inside it.
 *
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        <View className="mx-6 mt-4 bg-white rounded-2xl p-5 border border-border items-center">
          ...
          {email ? <Text className="text-text-muted text-sm mt-1">{email}</Text> : null}
          {profile?.role && (
